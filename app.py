...
db = client["footwear_db"]
collection = db["items"]

#  Main Window
root = tk.Tk()
root.title("💎 footwear_db Store")
...
# Input Fields
tk.Label(root, text="footwear_db Name:", font=("Helvetica", 12, "bold"), bg="#fefae0").pack(pady=5)
...
# ===== CRUD Functions =====
def create_item():
    """Add new footwear_db to MongoDB"""
    ...
    if name and price:
        try:
            ...
            if result.matched_count > 0:
                ...
                messagebox.showwarning("⚠ Not Found", f"No footwear_db found with name '{name}'")
        except ValueError:
            messagebox.showerror("❌ Error", "Price must be a number!")
    else:
...
def delete_item():
    """Delete footwear_db from MongoDB"""
    name = name_entry.get().strip()
    if name:
